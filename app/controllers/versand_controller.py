...
def send_managers(app, mode: str | None = None) -> None:
    ...
    from app.services.email_service import send_managers as email_send_managers
    try:
        logger.info("Versand: Start send_managers")
        email_send_managers(app, mode=mode)
...
def send_selected_employees(app, mode: str | None = None) -> None:
    ...
    from app.services.email_service import send_selected_employees as email_send_selected
    try:
        logger.info("Versand: Start send_selected_employees")
        email_send_selected(app, mode=mode)
        from tkinter import messagebox
...
def create_vg_ma_relationship(app) -> None:
    ...
    from app.services.sap_data_service import create_vg_ma_relationship as sap_create_relationship
    sap_create_relationship(app)


def refresh_mgr_table(app) -> None:
    """Aktualisiert die VG-Tabelle im Massenversand-Tab."""
    from app.services.sap_data_service import refresh_mgr_table as sap_refresh_mgr_table
    sap_refresh_mgr_table(app)


def refresh_mgr_table_einzel(app) -> None:
    """Aktualisiert die VG-Tabelle im Einzelversand-Tab."""
    from app.services.sap_data_service import refresh_mgr_table_einzel as sap_refresh_mgr_table_einzel
    sap_refresh_mgr_table_einzel(app)


def refresh_vg_list(app) -> None:
    """Aktualisiert die VG-Liste im VG-MA-Tab."""
    from app.services.sap_data_service import refresh_vg_list as sap_refresh_vg_list
    sap_refresh_vg_list(app)


def refresh_ma_list(app) -> None:
    """Aktualisiert die MA-Liste im VG-MA-Tab."""
    from app.services.sap_data_service import refresh_ma_list as sap_refresh_ma_list
    sap_refresh_ma_list(app)


def update_selection_status(app) -> None:
    """Aktualisiert den Auswahlinfo-Text im VG-MA-Tab."""
    from app.services.sap_data_service import update_selection_status as sap_update_selection_status
...
def preview_managers(app) -> None:
    ...
    from app.services.email_service import preview_managers as email_preview_managers
    try:
...
def preview_selected(app) -> None:
    ...
    from app.services.email_service import preview_selected as email_preview_selected
    try:
...
def render_mail_preview(app) -> None:
    ...
    from app.services.email_service import render_mail_preview as email_render_preview
    try:
