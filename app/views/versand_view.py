...
def build_versand(parent: ttk.Frame, app) -> None:
    ...
    # Tab 3: Temporäres VG-MA-Verhältnis
    app.frame_vg_ma = ttk.Frame(app.versand_notebook)
    app.versand_notebook.add(app.frame_vg_ma, text="Temporäres VG-MA-Verhältnis")
...
def build_massenversand(app) -> None:
    ...
    scrollbar_y = ttk.Scrollbar(app.frame_massenversand, orient="vertical", command=app.tree.yview)
    scrollbar_y.grid(row=2, column=6, sticky="ns")
    app.tree.configure(yscrollcommand=scrollbar_y.set)
    
    scrollbar_x = ttk.Scrollbar(app.frame_massenversand, orient="horizontal", command=app.tree.xview)
    scrollbar_x.grid(row=3, column=0, columnspan=6, sticky="ew", padx=8, pady=(0,8))
    app.tree.configure(xscrollcommand=scrollbar_x.set)

    btn_frame = ttk.Frame(app.frame_massenversand)
    btn_frame.grid(row=4, column=0, columnspan=6, sticky="ew", padx=8, pady=8)
    ...
    prog_frame.grid(row=5, column=0, columnspan=6, sticky="ew", padx=8, pady=(0,8))
...
def build_einzelversand(app) -> None:
    ...
    # Suchfeld für Vorgesetzte (wie im Massenversand)
    ttk.Label(toolbar, text="Suche:").pack(side="left", padx=(0, 4))
    app.filter_var_einzel = tk.StringVar()
    entry = ttk.Entry(toolbar, textvariable=app.filter_var_einzel, width=36)
    entry.pack(side="left", padx=(0, 8))
    from app.controllers.versand_controller import refresh_mgr_table_einzel
    entry.bind("<KeyRelease>", lambda e: refresh_mgr_table_einzel(app))
    ...
    app.tree_einzel.grid(row=2, column=0, columnspan=6, sticky="nsew", padx=8, pady=(0,0))
    app._bind_treeview_sort(app.tree_einzel, numeric_like={"PN", "Anzahl MA"})
    
    scrollbar_y_einzel = ttk.Scrollbar(app.frame_einzelversand, orient="vertical", command=app.tree_einzel.yview)
    scrollbar_y_einzel.grid(row=2, column=6, sticky="ns")
    app.tree_einzel.configure(yscrollcommand=scrollbar_y_einzel.set)
    
    scrollbar_x_einzel = ttk.Scrollbar(app.frame_einzelversand, orient="horizontal", command=app.tree_einzel.xview)
    scrollbar_x_einzel.grid(row=3, column=0, columnspan=6, sticky="ew", padx=8, pady=(0,8))
    app.tree_einzel.configure(xscrollcommand=scrollbar_x_einzel.set)

    ttk.Label(app.frame_einzelversand, text="Mitarbeitende:").grid(row=4, column=0, sticky="w", padx=8, pady=(0,4))
    ...
    app.subs_tree.grid(row=5, column=0, columnspan=6, sticky="nsew", padx=8, pady=(0,0))
    app._bind_treeview_sort(app.subs_tree, numeric_like={"PN"})
    
    scrollbar_y_subs = ttk.Scrollbar(app.frame_einzelversand, orient="vertical", command=app.subs_tree.yview)
    scrollbar_y_subs.grid(row=5, column=6, sticky="ns")
    app.subs_tree.configure(yscrollcommand=scrollbar_y_subs.set)
    
    scrollbar_x_subs = ttk.Scrollbar(app.frame_einzelversand, orient="horizontal", command=app.subs_tree.xview)
    scrollbar_x_subs.grid(row=6, column=0, columnspan=6, sticky="ew", padx=8, pady=(0,8))
    app.subs_tree.configure(xscrollcommand=scrollbar_x_subs.set)

    doc_frame = ttk.LabelFrame(app.frame_einzelversand, text="Dokumenttypen")
    doc_frame.grid(row=7, column=0, columnspan=6, sticky="ew", padx=8, pady=(0,8))
    ...
    btn_frame.grid(row=8, column=0, columnspan=6, sticky="ew", padx=8, pady=8)
    ...
    prog_frame_e.grid(row=9, column=0, columnspan=6, sticky="ew", padx=8, pady=(0,8))
    ...
    app.frame_einzelversand.grid_rowconfigure(5, weight=1)
...
def build_vg_ma_creation(app) -> None:
    """Erstellt den Tab für temporäre VG-MA-Verhältnisse."""
    toolbar = ttk.Frame(app.frame_vg_ma)
    toolbar.grid(row=0, column=0, columnspan=6, sticky="ew", padx=8, pady=8)

    create_info_button(
        parent=toolbar,
        title="Info • Temporäres VG-MA-Verhältnis",
        text=(
            "Temporäre Vorgesetzten-Mitarbeiter-Beziehung erstellen\n\n"
            "Wann benötigt:\n"
            "• Bei Vertretungen: Temporäre Zuordnung zu einem anderen VG\n"
            "• Bei fix veränderten Anstellungsverhältnissen stattdessen neuen\n" 
            "   SAP Export erstellen lassen\n\n"
            "Vorgehen:\n"
            ...
            "Was passiert:\n"
            "• Alle Stammdaten bleiben identisch (Name, OE, BsGrd, etc.)\n"
            "• Nur die VG-PN wird geändert\n"
            "• Die alte Zeile bleibt bestehen (Duplikat wird erstellt)\n\n"
            "Warnung:\n"
            "⚠️ Änderungen sind sofort in EXPORT.xlsx gespeichert!\n"
            "⚠️ Alte VG-MA-Beziehung bleibt parallel bestehen"
        ),
        side="right",
    )
    ...
    vg_search_entry = ttk.Entry(vg_search_frame, textvariable=app.vg_search_var, width=30)
    vg_search_entry.pack(side="left")
    ...
    vg_tree_frame = ttk.Frame(vg_frame)
    vg_tree_frame.pack(fill="both", expand=True, padx=8, pady=(0,8))
    
    scrollbar_y_vg = ttk.Scrollbar(vg_tree_frame, orient="vertical")
    scrollbar_y_vg.pack(side="right", fill="y")
    
    scrollbar_x_vg = ttk.Scrollbar(vg_tree_frame, orient="horizontal")
    scrollbar_x_vg.pack(side="bottom", fill="x")
    
    app.vg_tree = ttk.Treeview(vg_tree_frame, columns=["PN", "Nachname", "Vorname", "OE"], show="headings", height=8,
                                yscrollcommand=scrollbar_y_vg.set, xscrollcommand=scrollbar_x_vg.set)
    for c in ["PN", "Nachname", "Vorname", "OE"]:
    ...
    app.vg_tree.pack(side="left", fill="both", expand=True)
    app._bind_treeview_sort(app.vg_tree, numeric_like={"PN"})
    
    scrollbar_y_vg.config(command=app.vg_tree.yview)
    scrollbar_x_vg.config(command=app.vg_tree.xview)
    ...
    ma_search_entry = ttk.Entry(ma_search_frame, textvariable=app.ma_search_var, width=30)
    ma_search_entry.pack(side="left")
    ma_search_entry.bind("<KeyRelease>", lambda e: refresh_ma_list(app))

    ma_tree_frame = ttk.Frame(ma_frame)
    ma_tree_frame.pack(fill="both", expand=True, padx=8, pady=(0,8))
    
    scrollbar_y_ma = ttk.Scrollbar(ma_tree_frame, orient="vertical")
    scrollbar_y_ma.pack(side="right", fill="y")
    
    scrollbar_x_ma = ttk.Scrollbar(ma_tree_frame, orient="horizontal")
    scrollbar_x_ma.pack(side="bottom", fill="x")
    
    app.ma_tree = ttk.Treeview(ma_tree_frame, columns=["PN", "Nachname", "Vorname", "OE"], show="headings", height=8,
                                yscrollcommand=scrollbar_y_ma.set, xscrollcommand=scrollbar_x_ma.set)
    for c in ["PN", "Nachname", "Vorname", "OE"]:
    ...
    app.ma_tree.pack(side="left", fill="both", expand=True)
    app._bind_treeview_sort(app.ma_tree, numeric_like={"PN"})
    
    scrollbar_y_ma.config(command=app.ma_tree.yview)
    scrollbar_x_ma.config(command=app.ma_tree.xview)
    ...
    ttk.Button(btn_frame, text="Neue Beziehung erstellen", command=lambda: create_vg_ma_relationship(app), style='Primary.TButton').pack(side="left", padx=(0,8))
